...
const PORT = process.env.PORT || 3000
    ...
    app.post("/apod/newApod", async (req, res) => {  
        Apod.create(req.body).then(newApod => { // *CREATE* Add an entry to the db. Refer to NASA API for data.
        res.json(newApod);
        });
    });

    app.put("/apod/:id", async (req, res) => {
        Apod.findByIdAndUpdate( // *UPDATE* Find a specific entry by ID and update based on the body.
